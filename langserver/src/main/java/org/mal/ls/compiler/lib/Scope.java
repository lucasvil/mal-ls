...
/**
 * Scope stores maps on a stack. Each map is a 'scope' of values uniquely
 * identified by their map keys. Values from previous scopes may be retrieved
 * with the lookup() and lookdown() methods.
 */
public class Scope<T> {
  ...
  /**
   * Iterates the scopes from the bottom up (back to front), and returns the value
   * of the first map containing the key.
   *
  ...
  /**
   * Iterates the scope from the top down (front to back), and returns the value
   * of the first map containing the key.
   *
   * @param key Object key
   * @return Object associated with the first match of key, or null if not found
  ...
  /**
   * Looks only at the current scope and returns the value associated with the
   * key.
   *
   * @param key Object key
