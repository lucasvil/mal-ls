...
import org.mal.ls.formatter.blocks.Block;
import org.mal.ls.formatter.blocks.IndentBlock;
import org.mal.ls.formatter.blocks.LineBlock;
import org.mal.ls.formatter.blocks.MultiBlock;
import org.mal.ls.formatter.blocks.StackBlock;
import org.mal.ls.formatter.blocks.TextBlock;
import org.mal.ls.formatter.blocks.WrapBlock;
...
public class Parser {
  ...
  private Block blockify(String string, boolean stackBlockLast) {
    ...
    for (var comment : tok.preComments) {
      if (currentLine + 1 < comment.getStart().getLine()) {
        pre.add(new TextBlock(""));
        currentLine = comment.getStart().getLine();
      }
      if (comment.type == TokenType.SINGLECOMMENT) {
        pre.add(new TextBlock("//" + comment.stringValue));
        currentLine = comment.getStart().getLine();
      } else {
        int end = comment.getStart().getLine() + comment.stringValue.split("\\n", -1).length - 1;
        if (end == tok.getStart().getLine()) {
    ...
    if (currentLine + 1 < tok.getStart().getLine() && allowBreaks) {
      pre.add(new TextBlock(""));
      currentLine = tok.getStart().getLine();
    }
    ...
    for (var comment : tok.postComments) {
      if (comment.type == TokenType.SINGLECOMMENT) {
        ...
        lastSingle = false;
        currentLine = comment.getStart().getLine() + comment.stringValue.split("\\n", -1).length - 1;
      }
    }
  ...
  private void parseVariable(StackBlock block) {
    var let = blockify();
    allowBreaks = false;
    var def = new LineBlock(let, new TextBlock(" "), blockify(tok.stringValue), new TextBlock(" "), blockify());
  ...
  private void parseExprList(MultiBlock block) {
    // Transfer arrows post comment to contents precomments to avoid shifting future
    // stackblock
  ...
  private LineBlock parseMeta2(Block saved) {
    allowBreaks = false;
    var block = new LineBlock(saved, // info type
        new TextBlock(" "), blockify(), // info
        blockify(), // :
        new TextBlock(" "), blockify("\"" + tok.stringValue + "\"", true));
    allowBreaks = true;
    return block;
  }
